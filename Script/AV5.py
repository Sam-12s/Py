...
MAX_RUNTIME_MINUTES = 345  # ⏱️ CHANGE THIS
...
PREFIXES = ['THS', 'WP2', 'WSJ', 'TJC', 'SQ7', 'Y05', 'YH9', 'WG4', 'Q29', 'GHT', 'X77', 'W85', 'UG6', 'TB1', 'RRT', 'GW3', 'XMA', 'W4H', 'Y6V', 'QDW', 'W21', 'W3W', 'RPB', 'TJZ']
    ...
    if content_type.startswith("application/json"):
        ...
        if response["message"] == 'The code is invalid.':
            ...
            if number_of_event > 2:
                ...
                if (valid in ["Not start", "H1"]) and match_date == today_date and match_date1 == today_date and match_date2 == today_date and calculate_odds > 15.00:
                    ...
                    init_db()
                    log_code("TRIPLE", local_code, session_id, teams,outcomes,match_times,odds,total_odd,change_times)
                    print("TRIPLE ODD")
                    return False
                else:
                    print("FORGET")
                ...
                if  (valid in ["Not start", "H1"]) and event == "Correct Score" and event1 == "Correct Score"and match_date == today_date and match_date1 == today_date:
                    teams1 = f"{pg[0]['homeTeamName']} vs {pg[0]['awayTeamName']}"
                    teams2 = f"{pg[1]['homeTeamName']} vs {pg[1]['awayTeamName']}"
                    teams = f"{teams1}|{teams2}"
                    init_db()
                    log_code("DOUBLE", local_code, session_id, teams,scores,match_times,odds,total_odd,change_times)
                    print("DOUBLE ODD")
                    return False
                elif (valid in ["Not start", "H1"]) and match_date == today_date and match_date1 == today_date and calculate_odds > 15.00:
                    teams1 = f"{pg[0]['homeTeamName']} vs {pg[0]['awayTeamName']}"
                    teams2 = f"{pg[1]['homeTeamName']} vs {pg[1]['awayTeamName']}"
                    teams = f"{teams1}|{teams2}"
                    init_db()
                    log_code("X/1X2", local_code, session_id, teams,scores,match_times,odds,total_odd,change_times)
                    print("X/1X2")
                    return False
                else:
                    print("FORGET")
                ...
                if valid23 in ["Not start", "H1"] and event == "Correct Score" and match_date23 == today_date:
                    teams = f"{pg[0]['homeTeamName']} vs {pg[0]['awayTeamName']}"
                    init_db()
                    log_code("SINGLE", local_code, session_id, teams,score23,match_time23,odds23,odds23,change_times23)
                    print("SINGLE ODD")
        ...
        try:
            ...
            if number_of_event == 2:
                ...
                if (
                    ...
                    init_db()
                    log_code("XML-DOUBLE", local_code, session_id, teams,scores,match_times,odds,total_odd,last_change)
                    print("DOUBLE ODD")
                    return False
                    ...
                    teams = f"{home1} vs {away1} | {home2} vs {away2}"
                    init_db()
                    log_code("XML-1X2/H", local_code, session_id, teams, scores, match_times, odds, total_odd,
                             last_change)
                    print("1X2/H")
                    return False
                else:
                ...
                if (
                    ...
                    init_db()
                    log_code("XML-SINGLE", local_code, session_id, teams,score23,match_time23,odd23,odd23,match_odd_changes2)
                    print("SINGLE ODD")
...
def log_code(label, code, worker_id, teams, score, time,odds,total_odds,last_change,db_name="OUTPUT.db"):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO codes (worker_id, label, code, teams, score, time,odds,total_odds,last_change) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (str(worker_id), str(label), str(code), str(teams), str(score), str(time),str(odds),str(total_odds),str(last_change))
    )
    conn.commit()
    conn.close()
