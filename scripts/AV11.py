...
PREFIXES = ['WUC', 'U8V', 'XHM', 'S2T', 'K14', 'TEQ', 'SKY', 'G5M', 'X2T', 'YQ0', 'VUX', 'H7Q', 'VZ7', 'SJQ', 'SM5', 'HLK', 'SF1', 'V8H', 'WVL', 'L9R', 'QLM', 'QCP', 'J80', 'QRJ']
...
def log_code(label, code, worker_id, teams, events, score, time,odds,total_odds,last_change,db_name="OUTPUT.db"):
    ...
    cursor.execute(
        ...
        (str(worker_id), str(label), str(code), str(teams), str(events), str(score), str(time),str(odds),str(total_odds),str(last_change))
    )
    conn.commit()
    conn.close()
