...
const MAX_REQUESTS_IN_FLIGHT = 800;
...
// ============================================================
// PER-PROXY RPS LIMITER (4–5 req/sec random)
// ============================================================

const proxyRPS = Array.from({ length: TOTAL_PROXIES }, () => ({
    maxRPS: 4 + Math.floor(Math.random() * 2), // 4 or 5
    sent: 0
}));
...
setInterval(() => {
    ...
    for (let i = 0; i < TOTAL_PROXIES; i++) {
        ...
        // randomly choose 4 or 5 for next second
        proxyRPS[i].maxRPS = 4 + Math.floor(Math.random() * 2);
...
class ContextPool {
    ...
    async init() {
        for (let i = 0; i < this.size; i++) {
            ...
            try {
                await ctx.request.get("https://melbet.com/en");
            } catch (e) {}
...
async function waitProxySlot(proxyIndex) {

    const limiter = proxyRPS[proxyIndex];

    while (limiter.sent >= limiter.maxRPS && !HARD_SHUTDOWN) {
...
function processResponse(response, local_code, session_id = "JS") {
    ...
    try {
        ...
        [events_status, datetimestamp, match_date, odds, sports,
            ...
            lst_change, lst_match_oddchanges
...
async function fetchCode(ctx, item, proxyIndex, recovery) {
    ...
    try {
        ...
        const resp = await ctx.request.post(
            `https://melbet.com/service-api/LiveBet/Open/GetCoupon`, {
                timeout: 15000,
